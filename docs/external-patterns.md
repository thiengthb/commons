...
| Ink (React for the terminal), used WITHOUT JSX so the client needs no build step                                                             | **0 of 10** repos had any terminal UI. The decisive gap was not the library but the requirement: `'ばかりに'.length` is 4 and its terminal width is 8, so a hand-rolled table is crooked on every Japanese row | `sakubun` `cli/` (devDep, not vendored) |
...
| `commander` / `yargs` for the sakubun CLI                            | The command list is SERVED by the app (`GET /api/cli`) so the client generates its own help and flag parsing from it. A framework would mean declaring every subcommand a second time, statically, in the client — the exact drift the served catalog exists to prevent. `node:util parseArgs` covers the rest in ~40 lines. Guidance says reach for commander when you need subcommands/generated help; here both are generated.                    |
| OpenTUI (React TUI on a Zig core) instead of Ink                     | Faster and lighter than Ink on every axis measured, and **requires Bun**. Every fleet repo is Node-only, so adopting it means a second runtime for one client. Re-check if any project ever moves to Bun.                                                                                                                                                                                                                                          |
...
| 2026-08-01 | terminal UI (TUI) library for Node                | `ink`, `blessed`/`neo-blessed`, OpenTUI, hand-rolled ANSI          | **ADOPTED `ink`** (sakubun `cli/`, devDep) — decided by a MEASUREMENT: Japanese is double-width      |
| 2026-08-01 | CLI argument parsing                             | `node:util parseArgs`, `commander`, `yargs`, nodejs-cli-best-practices | **ADOPTED core `parseArgs`, refused commander** — the subcommand list is served by the app, not declared |
